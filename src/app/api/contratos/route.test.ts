...
describe('GET /api/contratos', () => {
  ...
  it('RC2 sin sesión → unauthorized', async () => {
    mockRequireOwnership.mockRejectedValue(new Error('UNAUTHORIZED'));
    await expect(
      GET(new Request('http://localhost/api/contratos?empresaId=e1'))
    ).rejects.toThrow('UNAUTHORIZED');
    expect(mockListar).not.toHaveBeenCalled();
  });

  it('RC3 ownership violado → forbidden', async () => {
    mockRequireOwnership.mockRejectedValue(new Error('FORBIDDEN'));
    await expect(
      GET(new Request('http://localhost/api/contratos?empresaId=e99'))
