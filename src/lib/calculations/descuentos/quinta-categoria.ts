...
function calcularImpuestoProgresivo(rentaImponible: number, uit: number): number {
  const tramos = [
...
export function calcularRetencionQuinta(
  ...
  const { remuneracionMensual, gratificacionesAnuales, mesActual, retencionesAnteriores, uit } = input;
  ...
  const impuestoAnual = calcularImpuestoProgresivo(rentaImponible, uit);
